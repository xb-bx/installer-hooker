...
void resume() {
  ...
  if(curl) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "http://127.0.0.1:8080/api/v2/torrents/resume");
...
void pause() {
  ...
  if(curl) {
    ...
    char params[512];
    params[sprintf(params, "hashes=%s", torrent_hash)] = 0;
    curl_easy_setopt(curl, CURLOPT_URL, "http://127.0.0.1:8080/api/v2/torrents/pause");
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, params);
    curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_data);
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, &buf);
...
void set_max_priority(int index, int prior) {
  ...
  if(curl) {
    struct response_buf buf = {0};
    char params[512];
    params[sprintf(params, "hash=%s&id=%i&priority=%i", torrent_hash, index, prior)] = 0;
    curl_easy_setopt(curl, CURLOPT_URL, "http://127.0.0.1:8080/api/v2/torrents/filePrio");
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, params);
    curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_data);
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, &buf);
...
BOOL check_torrent() {
      ...
      if(curl) {
        ...
        cJSON_ArrayForEach(torrent, json) {
            char *hash = cJSON_GetObjectItemCaseSensitive(torrent, "hash")->valuestring;
            if (strcmp(hash, torrent_hash) == 0){
                printf("Found\n");
                base_path = malloc(512);
                base_path[0] = 0;
                strcpy(base_path, cJSON_GetObjectItemCaseSensitive(torrent, "content_path")->valuestring);
                size_t len;
...
void __declspec(dllexport) wait_for_file(char *lpFileName) {
    if(is_downloaded(lpFileName)) return;
    if(torrent_hash == NULL) {
        while(torrent_hash == NULL) Sleep(100);
    }
    if(files_url == NULL) {
        if (!check_torrent()) {
        ...
        sprintf(files_url, "http://127.0.0.1:8080/api/v2/torrents/files?hash=%s", torrent_hash);
    }
    double progress = 0.0;
...
HANDLE __declspec(dllexport) __stdcall hookA (
  ...
  HANDLE                hTemplateFile
    ...
    for(int i = 0; i < 16; i++)
        addrA[i] = oldA[i];
    HANDLE res = CreateFileA(lpFileName, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile);
    for(int i = 0; i < 16; i++)
        addrA[i] = newA[i];
    return res;
}
...
HANDLE __declspec(dllexport) __stdcall hookWKB (
  ...
  HANDLE                hTemplateFile
    ...
    size_t len = utf16_to_utf8(lpFileName, lstrlenW(lpFileName), utf8filename, 512);
    utf8filename[len] = 0;
...
DWORD __declspec(dllexport) __stdcall hookFileAttrsW (
  LPCWSTR               lpFileName
) {
...
void __declspec(dllexport) create_hook(char *old, char *new, char *addr, void* hook) {
    int offset = 0;
#if defined _M_IX86
    addr[offset + 0] = 0xb8;
...
/*int main() {*/
    if (_fdwReason == DLL_PROCESS_ATTACH) {
        ...
        addrAttrsW = GetProcAddress(GetModuleHandle("kernel32.dll"), "GetFileAttributesW");
        for(int i = 0; i < 16; i++)
            attrsOldW[i] = addrAttrsW[i];
        old_protect = 0;
        if(!VirtualProtect(addrAttrsW, 4096, PAGE_EXECUTE_READWRITE, &old_protect)) {
            printf("error\n");
        }
