...
void __declspec(dllexport) wait_for_file(char *lpFileName) {
    ...
    if (files_url == NULL) {
        ...
        sprintf(files_url,
                "http://127.0.0.1:8080/api/v2/torrents/files?hash=%s",
                torrent_hash);
    }
    double progress = 0.0;
...
void __declspec(dllexport) create_hook(char *old, char *new, char *addr,
                                       void *hook) {
    int offset = 0;
#if defined _M_IX86
    addr[offset + 0] = 0xb8;
        ...
        LPVOID _lpReserved)  // reserved
{
    if (_fdwReason == DLL_PROCESS_ATTACH) {
        ...
        for (int i = 0; i < 16; i++)
            oldW[i] = addrW[i];
        DWORD old_protect = 0;
        if (!VirtualProtect(addrW, 4096, PAGE_EXECUTE_READWRITE,
                            &old_protect)) {
            printf("error\n");
        }
        ...
        addrWKB =
            GetProcAddress(GetModuleHandle("kernelbase.dll"), "CreateFileW");
        for (int i = 0; i < 16; i++)
            oldWKB[i] = addrWKB[i];
        old_protect = 0;
        if (!VirtualProtect(addrWKB, 4096, PAGE_EXECUTE_READWRITE,
                            &old_protect)) {
            printf("error\n");
        }
        ...
        for (int i = 0; i < 16; i++)
            oldA[i] = addrA[i];
        old_protect = 0;
        if (!VirtualProtect(addrA, 4096, PAGE_EXECUTE_READWRITE,
                            &old_protect)) {
            printf("error\n");
        }
